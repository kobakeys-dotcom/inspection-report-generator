...
const OtherRfiDashboard = () => {
  const navigate = useNavigate();
  const [rfis, setRfis] = useState<OtherRfiFormData[]>([]);
  const [search, setSearch] = useState('');
  const [loading, setLoading] = useState(true);

  const fetchRfis = async () => {
    try {
      const data = await otherRfiApi.getAll(search);
      setRfis(data);
    } catch {
      setRfis([]);
    } finally {
  ...
  const handleDelete = async (id: number, inspectionNo: number) => {
    if (!confirm(`Delete Other RFI IR-${inspectionNo}?`)) return;
    try {
      await otherRfiApi.delete(id);
  ...
  const handleExport = async (id: number) => {
    try {
      const rfi = await otherRfiApi.getById(id);
      await generateOtherRfiExcel(rfi);
    ...
    <div className="min-h-screen bg-background">
      <div className="bg-primary text-primary-foreground">
        <div className="max-w-6xl mx-auto px-4 py-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl font-bold">Other RFI</h1>
          </div>
          <p className="text-primary-foreground/80 text-sm">Request for Inspection — Other Works</p>
        </div>
      </div>
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <Button onClick={() => navigate('/other-rfi/new')}><Plus className="h-4 w-4 mr-1" />Create New Other RFI</Button>
        </div>
        {loading ? (
          ...
          <div className="text-center py-16 border border-dashed border-border rounded-lg">
            ...
            <h2 className="text-lg font-semibold text-foreground mb-1">No Other RFIs yet</h2>
            <p className="text-sm text-muted-foreground mb-4">{search ? 'No results found.' : 'Create your first Other RFI to get started.'}</p>
            {!search && <Button onClick={() => navigate('/other-rfi/new')}><Plus className="h-4 w-4 mr-1" />Create New Other RFI</Button>}
          </div>
        ) : (
                  ...
                  <tr key={rfi.id} className="border-t border-border hover:bg-muted/30 transition-colors">
                    ...
                    <td className="p-3">
                      <div className="flex items-center justify-end gap-1">
                        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => navigate(`/other-rfi/edit/${rfi.id}`)} title="Edit"><Pencil className="h-4 w-4" /></Button>
...
export default OtherRfiDashboard;
