import { OtherRfiFormData, OTHER_PROJECT_INFO } from '@/types/otherRfi';
import hdcLogo from '@/assets/hdc-logo.png';
import bltLogo from '@/assets/blt-logo.png';

interface Props {
  data: OtherRfiFormData;
  onChange: (data: Partial<OtherRfiFormData>) => void;
}
...
const OtherRfiPage1 = ({ data, onChange }: Props) => {
  return (
    <div className="bg-white text-black border-[0.5px] border-[#888]" style={{ fontFamily: "Calibri, 'Segoe UI', Arial, sans-serif", fontSize: '10px', lineHeight: '1.3' }}>
      <div className="p-[20px]">
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <div className="text-[8px] mt-[1px]">{OTHER_PROJECT_INFO.form_number}</div>
          </div>
        ...
        {/* Project Details + BLT logo */}
        <div className="flex items-start mb-[6px]">
          <table className="border-collapse" style={{ width: '58%' }}>
            <tbody>
              <tr>
                <td colSpan={2} className="bg-[#4CAF50] text-white text-[10px] font-semibold px-[6px] py-[3px]">Project Details</td>
              </tr>
              {[
                ['Project', OTHER_PROJECT_INFO.project],
                ['Contractor', OTHER_PROJECT_INFO.contractor],
                ['Contract No', OTHER_PROJECT_INFO.contract_no],
                ['Client', OTHER_PROJECT_INFO.client],
              ].map(([label, value], i) => (
                <tr key={label}>
                  <td className={`border-[0.5px] border-[#888] ${i === 0 ? 'border-t-0' : ''} px-[6px] py-[3px] text-[10px] leading-[14px] font-semibold`} style={{ width: '100px' }}>{label}</td>
                  <td className={`border-[0.5px] border-[#888] ${i === 0 ? 'border-t-0' : ''} px-[6px] py-[3px] text-[10px] leading-[14px]`}>{value}</td>
                </tr>
              ))}
            </tbody>
          </table>
              ...
              {(['received_by_name', 'received_by_designation', 'received_by_date'] as const).map((field, i) => (
                <div key={field} className="flex items-center gap-[4px]">
                  ...
                  <div className="flex-1 border-b border-[#333]">
                    <input type={field === 'received_by_date' ? 'date' : 'text'} className="w-full bg-transparent border-0 px-0 py-0 text-[10px] leading-[16px] focus:outline-none" value={data[field]} onChange={(e) => onChange({ [field]: e.target.value })} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <table className="w-full border-collapse mb-[6px]">
          <tbody>
            ...
            {[1, 2, 3, 4, 5].map((num) => {
              const key = `inspection_item_${num}` as keyof OtherRfiFormData;
              return (
                <tr key={num}>
            ...
            <div style={{ width: '50%' }} className="space-y-[6px]">
              {(['pre_inspection_name', 'pre_inspection_designation', 'pre_inspection_date'] as const).map((field, i) => (
                <div key={field} className="flex items-center gap-[4px]">
                  <span className="text-[10px] font-semibold w-[70px]">{['Name:', 'Designation:', 'Date:'][i]}</span>
                  <div className="flex-1 border-b border-[#333]">
                    <input type={field === 'pre_inspection_date' ? 'date' : 'text'} className="w-full bg-transparent border-0 px-0 py-0 text-[10px] leading-[16px] focus:outline-none" value={data[field]} onChange={(e) => onChange({ [field]: e.target.value })} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <table className="w-full border-collapse mb-[6px]">
          <tbody>
            <tr><td colSpan={2} className="bg-[#4CAF50] text-white text-[10px] font-semibold px-[6px] py-[3px] border-[0.5px] border-[#888] text-center">Comments (URBANCO USE ONLY):</td></tr>
            <tr>
              <td className={td} style={{ width: '140px', verticalAlign: 'top' }}><span className="text-[9px]">Relevant Sub-clause/Term</span></td>
              <td className={td} rowSpan={2}><textarea className={redInp + " min-h-[50px]"} value={data.comments} onChange={(e) => onChange({ comments: e.target.value })} rows={3} /></td>
            </tr>
            ...
            <div style={{ width: '50%' }} className="pl-[8px] space-y-[6px]">
              {(['client_rep_name', 'client_rep_designation', 'client_rep_date'] as const).map((field, i) => (
                <div key={field} className="flex items-center gap-[4px]">
                  <span className="text-[10px] font-semibold w-[70px]">{['Name:', 'Designation:', 'Date:'][i]}</span>
                  <div className="flex-1 border-b border-[#333]">
                    <input type={field === 'client_rep_date' ? 'date' : 'text'} className="w-full bg-transparent border-0 px-0 py-0 text-[10px] leading-[16px] focus:outline-none" value={data[field]} onChange={(e) => onChange({ [field]: e.target.value })} />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ width: '50%' }} className="space-y-[6px]">
              {(['contractor_rep_name', 'contractor_rep_designation', 'contractor_rep_date'] as const).map((field, i) => (
                <div key={field} className="flex items-center gap-[4px]">
                  <span className="text-[10px] font-semibold w-[70px]">{['Name:', 'Designation:', 'Date:'][i]}</span>
                  <div className="flex-1 border-b border-[#333]">
                    <input type={field === 'contractor_rep_date' ? 'date' : 'text'} className="w-full bg-transparent border-0 px-0 py-0 text-[10px] leading-[16px] focus:outline-none" value={data[field]} onChange={(e) => onChange({ [field]: e.target.value })} />
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="text-right text-[8px] text-[#999] px-[20px] pb-[8px]">Page 1 of 1</div>
    </div>
  );
};

export default OtherRfiPage1;
