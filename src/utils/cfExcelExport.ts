...
export async function generateCfRfiExcel(data: CladdingFrameRfiFormData) {
  const workbook = new ExcelJS.Workbook();

  const response = await fetch('/RFI-Cladding-Frame.xlsx');
  ...
  a.href = url;
  a.download = `RFI-CF-IR-${data.inspection_no ?? 'draft'}.xlsx`;
