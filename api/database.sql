-- RFI Generator Database Schema (Updated)
...
CREATE TABLE IF NOT EXISTS cf_rfis (
    id INT AUTO_INCREMENT PRIMARY KEY,
    inspection_no INT NOT NULL UNIQUE,
    ref_drawing VARCHAR(255) DEFAULT '',
    work_site VARCHAR(255) DEFAULT '',
...
CREATE TABLE IF NOT EXISTS cf_rfi_checklist_items (
    id INT AUTO_INCREMENT PRIMARY KEY,
    rfi_id INT NOT NULL,
    item_order INT NOT NULL,
    description TEXT NOT NULL,
    result ENUM('pass', 'fail', 'na', '') DEFAULT '',
    item_comments TEXT DEFAULT '',
    FOREIGN KEY (rfi_id) REFERENCES cf_rfis(id) ON DELETE CASCADE
);

-- Shares the same counter as Cladding RFI (all RFI types use one counter)

INSERT INTO default_checklist_items (item_order, description) VALUES
(101, 'All cladding frames installed as per approved drawings and specifications'),
(102, 'Frames are vertically and horizontally aligned'),
(103, 'No deformation or movement in the frame after installation'),
(104, 'Box bars are free from rust, corrosion, and any surface damage'),
(105, 'No sharp edges or protrusions that pose a safety risk'),
(106, 'All weld joints cleaned properly after welding (e.g., removal of slag, spatter, oil, or dust)'),
(107, 'Anti-corrosive coating applied evenly and covers the entire weld area'),
(108, 'No flaking, peeling, or damage to the applied anti-corrosion coating');

-- ============================================================
-- OTHER RFI (no checklist / page 2)
-- ============================================================

CREATE TABLE IF NOT EXISTS other_rfis (
    id INT AUTO_INCREMENT PRIMARY KEY,
    inspection_no INT NOT NULL UNIQUE,
    ref_drawing VARCHAR(255) DEFAULT '',
    work_site VARCHAR(255) DEFAULT '',
